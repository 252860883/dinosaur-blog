...
<p>上面已经提到了 Vue 通过<code>Object.defineProperty()</code>进行数据劫持，但是<code>Object.defineProperty()</code>有个缺陷，只能对属性进行数据劫持，所以不能对整个对象或是数组进行劫持。对此，Vue 也对数组和对象类型进行了一些增强处理，即通过递归遍历来实现：</p>
...
<p>从上面源码可以看出，Vue 对 Array 几个方法进行重写，除了执行数组原始方法以外，还会额外执行<code>ob.dep.notify()</code>，在这里就是很关键的实现响应式了。但是到这一步我们<strong>通过索引设置一个数组项或者改变数组的长度仍然无法实现响应式</strong>：</p>
